...
export class AutoFulfillmentService {
  ...
  async planOrder(orderId: string): Promise<number> {
    ...
    let rewardCoinsLeft = rewardCoinsOf(fulfillments[0]?.order.metadata) + campaignCoinsOf(fulfillments[0]?.order.metadata);
...
/** Coins the launch welcome benefit adds, from the same record. */
function campaignCoinsOf(metadata: unknown): number {
  const value = metadata && typeof metadata === 'object' ? (metadata as Record<string, unknown>)['campaignCoins'] : undefined;
  return typeof value === 'number' && value > 0 ? Math.round(value) : 0;
}
