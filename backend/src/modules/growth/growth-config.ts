...
export function sanitizeRewardTemplate(value: unknown, path: string): RewardTemplate {
  ...
  if (kind === 'OFFER_UNLOCK') {
    // Nothing in the catalog reads an unlock yet, so a card of this kind
    // would sit in the wallet doing nothing. Refused until an offer can honour it.
    throw new GrowthConfigError(`${path}.kind: OFFER_UNLOCK has no offer to unlock yet`);
  }
  const template: RewardTemplate = {
    kind,
    value: int(value['value'], `${path}.value`, 1, 100_000_000),
