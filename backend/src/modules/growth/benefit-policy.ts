 ...
 *   FIRST_ORDER   the launch welcome benefit (FIRST KICK): bonus coins on a
 *                 customer's first paid order, decided by the server.
 *   LOYALTY       a tier benefit, when the owner configures one.
 *   COUPON        a code typed into the cart: a coupon or a creator code.
 *
 * The rule in one line: earned things combine with the launch bonus and with
 * the welcome benefit; a coupon combines with nothing. One reward per order.
 */
export type BenefitKind = 'LAUNCH_BONUS' | 'REWARD' | 'FIRST_ORDER' | 'LOYALTY' | 'COUPON';
...
const COMBINES_WITH: Readonly<Record<BenefitKind, readonly BenefitKind[]>> = {
  LAUNCH_BONUS: ['REWARD', 'FIRST_ORDER', 'LOYALTY'],
  REWARD: ['LAUNCH_BONUS', 'FIRST_ORDER', 'LOYALTY'],
  FIRST_ORDER: ['LAUNCH_BONUS', 'REWARD', 'LOYALTY'],
  LOYALTY: ['LAUNCH_BONUS', 'REWARD', 'FIRST_ORDER'],
  COUPON: [],
};
 ...
 */
const PRECEDENCE: readonly BenefitKind[] = ['LAUNCH_BONUS', 'REWARD', 'FIRST_ORDER', 'LOYALTY', 'COUPON'];
...
const NAMES: Readonly<Record<BenefitKind, { he: string; en: string }>> = {
  ...
  FIRST_ORDER: { he: 'הטבת ההצטרפות', en: 'the welcome benefit' },
