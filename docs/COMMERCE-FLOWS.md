...
The server decides which benefits apply to an order (`backend/src/modules/growth/benefit-policy.ts`, run inside `PricingService.total`). Benefit kinds: `LAUNCH_BONUS` (the bonus coins carried by a bundle variant), `REWARD` (an earned EasyDrop / referral / EasyBack reward chosen in the cart by `rewardId`), `LOYALTY` (tier-derived, reserved), `COUPON`. Compatibility: LAUNCH_BONUS combines with REWARD and LOYALTY; REWARD with LAUNCH_BONUS and LOYALTY; COUPON combines with nothing. Precedence LAUNCH_BONUS > REWARD > LOYALTY > COUPON, one benefit per kind. The cart answer carries `benefits: { applied[], rejected[], rewardCoins }` so the storefront can say which benefit is on the order and why another is not (`COUPON_NOT_COMBINABLE`, `REWARD_NOT_APPLICABLE`, `COUPON_NOT_APPLICABLE`). The checkout session freezes that decision in `benefitsSnapshot`; order creation reserves the reward, settlement redeems it, cancellation or refund before settlement releases it. Reward coins are delivered once per order by auto-fulfillment together with the line coins and the launch bonus. The mock data layer mirrors the matrix. See `docs/GROWTH-AND-REWARDS.md` for the programmes that issue rewards.
